...
void ln_func(const int32_t *x, const int16_t *weight, const int32_t *bias,
    ...
    double mean_f = sum / (double)N / 1048576.0;  /* dequant */

    /* Compute var */
    double var_sum = 0.0;
    for (int i = 0; i < N; i++) {
        double d = (double)x[i] / 1048576.0 - mean_f;
        var_sum += d * d;
    }
    double var_f = var_sum / (double)N;
    double inv_std = 1.0 / sqrt(var_f + 1e-8);

    /* Re-quantize */
    int32_t mean_q = (int32_t)round(mean_f * 1048576.0);  /* s32f20 */
    uint16_t var_q = (uint16_t)round(inv_std * 2048.0);   /* u16f11 */
...
void log_gen_fixed(const int32_t *real, const int32_t *imag, int W, int32_t *out) {
    for (int w = 0; w < W; w++) {
        double r = (double)real[w] / 1048576.0;
        double im = (double)imag[w] / 1048576.0;
        double mag = sqrt(r * r + im * im);
        if (mag < 1e-12) mag = 1e-12;
        double log_val = log10(mag);
        /* Quantize to s32f20 */
        int64_t q = (int64_t)round(log_val * 1048576.0);
        out[w] = sat_i32(q);
    }
}

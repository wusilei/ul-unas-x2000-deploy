...
max_frames = min(T, 5);  % frames 0-4 sufficient for golden test
for t = 1:max_frames
    fprintf('Frame %d/%d\n', t, max_frames);
    ...
    % Save STFT input (float, for C test to load)
    write_bin([out_dir 'frame' num2str(t-1) '_stft_real.bin'], single(cmp_real(t, :)));
    write_bin([out_dir 'frame' num2str(t-1) '_stft_imag.bin'], single(cmp_imag(t, :)));
    ...
    write_bin([out_dir 'frame' num2str(t-1) '_e1_pconv0.bin'], int32(y_e1_pconv0));  % PConv0+BN+AP output
    y_e1_s = zeros(24, 65);
    y_e1_s(1:2:end, :) = y_e1_pconv0(1:12, :);
    y_e1_s(2:2:end, :) = y_e1_pconv0(13:24, :);
    write_bin([out_dir 'frame' num2str(t-1) '_e1_shuf.bin'], int32(y_e1_s));         % shuffle output

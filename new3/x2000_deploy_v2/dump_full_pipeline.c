...
static void dump_i32(const char *path, const int32_t *data, int n) {    FILE *f = fopen(path, "wb"); if (!f) return;    fwrite(data, sizeof(int32_t), n, f); fclose(f);}static void dump_u16(const char *path, const uint16_t *data, int n) {    FILE *f = fopen(path, "wb"); if (!f) return;    fwrite(data, sizeof(uint16_t), n, f); fclose(f);}
...
{ char dp[512]; snprintf(dp, sizeof(dp), "dump_c/frame%d_bm_c.bin", frame); dump_i32(dp, x_bm, 129); }
        ...
        /* Step 3a: E0 sub-block golden (before encoder to use correct cache state) */
        {
            ...
            if (frame == 0) {
                ...
                int32_t h_cache_ta_diag[24] = {0};
                gru_module_q20(ta_agg, TA_nH, TA_C, h_cache_ta_diag,
                           encoder_en_convs_0_ops_4_ta_gru_weight_ih_l0,
                           encoder_en_convs_0_ops_4_ta_gru_bias_ih_l0,
                           encoder_en_convs_0_ops_4_ta_gru_weight_hh_l0,
                           encoder_en_convs_0_ops_4_ta_gru_bias_hh_l0,
            ...
            if (frame == 0) {
                ...
                int32_t h_fwd_diag[4] = {0};
                for (int t = 0; t < FA_S; t++) {
                    gru_module_q20(&fa_reshaped[t * FA_G], FA_nH, FA_G, h_fwd_diag,
                               encoder_en_convs_0_ops_4_fa_gru_weight_ih_l0,
                               encoder_en_convs_0_ops_4_fa_gru_bias_ih_l0,
                               encoder_en_convs_0_ops_4_fa_gru_weight_hh_l0,
                               encoder_en_convs_0_ops_4_fa_gru_bias_hh_l0,
                ...
                int32_t h_rev_diag[4] = {0};
                for (int t = 0; t < FA_S; t++) {
                    int t_rev = FA_S - 1 - t;
                    gru_module_q20(&fa_reshaped[t_rev * FA_G], FA_nH, FA_G, h_rev_diag,
                               encoder_en_convs_0_ops_4_fa_gru_weight_ih_l0_reverse,
                               encoder_en_convs_0_ops_4_fa_gru_bias_ih_l0_reverse,
                               encoder_en_convs_0_ops_4_fa_gru_weight_hh_l0_reverse,
                               encoder_en_convs_0_ops_4_fa_gru_bias_hh_l0_reverse,
...
{ char dp[512]; snprintf(dp, sizeof(dp), "dump_c/frame%d_e0_c.bin", frame); dump_i32(dp, e0, 12*65); snprintf(dp, sizeof(dp), "dump_c/frame%d_e1_c.bin", frame); dump_i32(dp, e1, 24*33); snprintf(dp, sizeof(dp), "dump_c/frame%d_e2_c.bin", frame); dump_i32(dp, e2, 24*33); snprintf(dp, sizeof(dp), "dump_c/frame%d_e3_c.bin", frame); dump_i32(dp, e3, 32*33); snprintf(dp, sizeof(dp), "dump_c/frame%d_e4_c.bin", frame); dump_i32(dp, e4, 16*33); }
            ...
            for (int layer = 0; layer < 5; layer++) {
                ...
                if (layer == 1) {
                     ...
                     * ================================================ */
                    {
                        ...
                        int32_t h_fwd[4] = {0};
                        for (int t = 0; t < FA_S; t++) {
                            gru_module_q20(&fa_reshaped[t * FA_G], FA_nH, FA_G, h_fwd,
                        ...
                        for (int t = 0; t < FA_S; t++) {
                            int t_rev = FA_S - 1 - t;
                            gru_module_q20(&fa_reshaped[t_rev * FA_G], FA_nH, FA_G, h_rev,
...
{ char dp[512]; snprintf(dp, sizeof(dp), "dump_c/frame%d_rnn1_c.bin", frame); dump_i32(dp, r1, 16*33); }
        gdprnn_module(r1, st.inter_cache_1, 1, r2);
{ char dp[512]; snprintf(dp, sizeof(dp), "dump_c/frame%d_rnn2_c.bin", frame); dump_i32(dp, r2, 16*33); }
        ...
        if (frame == 0) {
            ...
            int32_t inter_cache_diag[33 * 16];
            memset(inter_cache_diag, 0, 33 * 16 * sizeof(int32_t));
            for (int t = 0; t < 33; t++) {
                int32_t h0[8], h1[8];
                memcpy(h0, &inter_cache_diag[t * 8], 8 * sizeof(int32_t));
                memcpy(h1, &inter_cache_diag[33 * 8 + t * 8], 8 * sizeof(int32_t));
                gru_module_q20(&xi0[t * 8], 8, 8, h0,
                ...
                memcpy(&inter_cache_diag[t * 8], h0, 8 * sizeof(int32_t));
                memcpy(&inter_cache_diag[33 * 8 + t * 8], h1, 8 * sizeof(int32_t));
            }
...
{ char dp[512]; snprintf(dp, sizeof(dp), "dump_c/frame%d_dec_c.bin", frame); dump_i32(dp, y_dec, 129); }
...
{ char dp[512]; snprintf(dp, sizeof(dp), "dump_c/frame%d_bs_c.bin", frame); dump_i32(dp, (int32_t*)y_bs, 257); }

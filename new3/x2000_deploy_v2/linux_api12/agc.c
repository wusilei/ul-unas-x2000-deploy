/**
 * agc.c — 语音 AGC + 降噪联合模块 (linux_api11)
 * ==============================================
 * voice_NoiseReductionAndAGC 内部: 纯后置AGC (NR→AGC)
 * 前置增益/能量判决在 ulunas_linux.c 中完成
 *
 * state_flag:
 *   0 = bypass
 *   1 = 只降噪 (DENOISE)
 *   2 = 只AGC (voice_AGC LMS + 固定增益)
 *   3 = 降噪 + AGC (后置AGC: NR→AGC)
 */
#include "agc.h"
...
/* ── 三级能量门控决策 ── */
void agc_gate_decision(unsigned short raw_energy,
                       long long *goal, int *gain_num, int *gain_den)
{
    if (raw_energy > ENERGY_VOICE_THR) {
        *goal     = AGC_GOAL_HIGH;
        *gain_num = AGC_GAIN_HIGH_NUM;
        *gain_den = AGC_GAIN_DEN;
    } else if (raw_energy > ENERGY_FAR_THR) {
        *goal     = AGC_GOAL_LOW;
        *gain_num = AGC_GAIN_LOW_NUM;
        *gain_den = AGC_GAIN_DEN;
    } else {
        *goal     = (1LL << 10);   /* noise floor: strong attenuation */
        *gain_num = 1;
        *gain_den = 2;             /* 0.5× */
    }
}

/* ── voice_NoiseReductionAndAGC: 纯后置AGC ── */
    ...
    long long goal, short flag, int n_samples)
{
    ...
    case STATE_FLAG_AGC: {
        short local[n_samples]; memcpy(local, in, n_samples * sizeof(short));
        int vf = g_v_inited ? 1 : 0;
        voice_agc_core(local, local, n_samples, (unsigned int)vf, goal);
        if (!g_first_frame) {
            int gn, gd;
            agc_gate_decision(0, &goal, &gn, &gd);  /* goal unused, just get gain */
            /* Use the goal's gain or default 2.5× */
            /* Actually, gate decision was already made by caller. Use fixed gain. */
            for (int i = 0; i < n_samples; i++) {
                int32_t v = (int32_t)local[i] * AGC_GAIN_HIGH_NUM / AGC_GAIN_DEN;
                out[i] = s16_clamp(v);
            }
    ...
    case STATE_FLAG_BOTH: {
        /* 纯后置AGC: NR → LMS AGC → 固定增益 */
        short nr_out[n_samples];
        noise_reduction(in, nr_out, n_samples);

        int vf = g_v_inited ? 1 : 0;
        voice_agc_core(nr_out, nr_out, n_samples, (unsigned int)vf, goal);

        if (!g_first_frame) {
            int gn, gd;
            agc_gate_decision(0, &goal, &gn, &gd);
            for (int i = 0; i < n_samples; i++) {
                int32_t v = (int32_t)nr_out[i] * gn / gd;
                out[i] = s16_clamp(v);
            }
        } else {

...
void voice_NoiseReductionAndAGC(short *in, short *out, long long goal, short flag, int n_samples)
{
    ...
    case STATE_FLAG_BOTH: {
        /* 混合AGC: 前置LMS归一化 → 降噪 → 后置LMS+固定增益+噪声门
         * 前置AGC让NR模型接收一致的电平, 后置AGC放大干净的语音 */
        short buf[400];
        memcpy(buf, in, n_samples * sizeof(short));

        /* 1. 前置AGC: LMS自适应归一化 (只用LMS, 不加固定增益) */
        int vf = g_v_inited ? 1 : 0;
        voice_agc_core(buf, buf, n_samples, (unsigned int)vf, goal);

        /* 2. 降噪 (输入已被AGC归一化, 匹配模型预期电平) */
        short nr_out[400];
        ...
        if (!gated) {
            /* 4. 后置AGC: LMS+固定增益 (NR输出已是干净语音, 安全放大) */
            voice_agc_core(nr_out, nr_out, n_samples, (unsigned int)vf, goal);
        }

        /* 5. 固定增益 + 噪声门绕过 */
        if (!g_first_frame) {
            int gn = gated ? AGC_GAIN_DEN
                     : (goal == AGC_GOAL_HIGH) ? AGC_GAIN_HIGH_NUM : AGC_GAIN_LOW_NUM;
            for (int i = 0; i < n_samples; i++) {

...
void voice_NoiseReductionAndAGC(short *in, short *out, long long goal, short flag, int n_samples)
{
    ...
    case STATE_FLAG_BOTH: {
        /* 混合AGC v2: 前置LMS归一化 → 降噪 → 后置LMS(始终) + 软门控增益 */
        short buf[400];
        memcpy(buf, in, n_samples * sizeof(short));

        /* 1. 前置AGC: LMS自适应归一化 */
        int vf = g_v_inited ? 1 : 0;
        voice_agc_core(buf, buf, n_samples, (unsigned int)vf, goal);

        /* 2. 降噪 */
        short nr_out[400];
        noise_reduction(buf, nr_out, n_samples);

        /* 3. 噪声门检测 */
        unsigned int nr_abs_sum = 0;
        for (int i = 0; i < n_samples; i++)
            nr_abs_sum += (nr_out[i] < 0) ? (unsigned short)(-nr_out[i]) : (unsigned short)nr_out[i];
        unsigned short nr_avg = (unsigned short)(nr_abs_sum / n_samples);
        int gated = (nr_avg < NOISE_GATE_THR);

        /* 4. 后置AGC: LMS始终运行, 自适应追踪信号电平 */
        voice_agc_core(nr_out, nr_out, n_samples, (unsigned int)vf, goal);

        /* 5. 固定增益: 软门控 — 低电平时减半增益而非旁路 */
        if (!g_first_frame) {
            int gn;
            if (gated) {
                gn = (goal == AGC_GOAL_HIGH) ? (AGC_GAIN_HIGH_NUM/2) : (AGC_GAIN_LOW_NUM/2);
                if (gn < AGC_GAIN_DEN) gn = AGC_GAIN_DEN;  /* 最低 1.0x */
            } else {
                gn = (goal == AGC_GOAL_HIGH) ? AGC_GAIN_HIGH_NUM : AGC_GAIN_LOW_NUM;
            }
            for (int i = 0; i < n_samples; i++) {

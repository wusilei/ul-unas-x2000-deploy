...
void voice_NoiseReductionAndAGC(short *in, short *out, long long goal, short flag, int n_samples)
{
    ...
    case STATE_FLAG_BOTH: {
        /* GTCRN v15 风格混合AGC: 前置固定1.5× → 降噪 → 后置LMS+软门控
         * 前置不用LMS, 避免前后共享状态互相干扰; LMS专注后置放大干净语音 */
        #define PRE_GAIN_NUM 3   /* 3/2=1.5x, GTCRN验证的轻量前置增益 */
        #define PRE_GAIN_DEN 2

        /* 1. 前置轻量固定增益 (不做LMS, 不污染状态) */
        short pre_out[400];
        for (int i = 0; i < n_samples; i++) {
        ...
        if (!g_first_frame) {
            ...
            if (gated) {
                ...
                if (gn < AGC_GAIN_DEN) gn = AGC_GAIN_DEN;
            } else {

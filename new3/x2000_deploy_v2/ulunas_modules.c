...
void inter_rnn_module(const int32_t *x, int32_t *h_cache, int gdprnn_idx, int32_t *y) {
    ...
    for (int t = 0; t < 33; t++) {
        ...
        memcpy(h0, &h_cache[t * 8], 8 * sizeof(int32_t));
        memcpy(h1, &h_cache[33 * 8 + t * 8], 8 * sizeof(int32_t));

        gru_module_q20(&x0[t * 8], 8, 8, h0,
                   rnn1_ih_w, rnn1_ih_b, rnn1_hh_w, rnn1_hh_b,
                   DPRNN_GRU_QR1, DPRNN_GRU_QR2, &x0_gru[t * 8]);

        gru_module_q20(&x1[t * 8], 8, 8, h1,
                   rnn2_ih_w, rnn2_ih_b, rnn2_hh_w, rnn2_hh_b,
                   DPRNN_GRU_QR1, DPRNN_GRU_QR2, &x1_gru[t * 8]);

        /* Store per-time-step output hidden state back to persistent cache */
        memcpy(&h_cache[t * 8], h0, 8 * sizeof(int32_t));
        memcpy(&h_cache[33 * 8 + t * 8], h1, 8 * sizeof(int32_t));
    }

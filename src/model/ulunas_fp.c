/**
 * denoising_fp.c — DENOISING Fixed-Point Operator Library
 * ===================================================
...
void denoising_state_init(denoising_state_t *st) {
    memset(st, 0, sizeof(denoising_state_t));
}
...
void bigru_module(const int32_t *x, int T, int nHidden, int in_dim,
    ...
    static int16_t y_rev_buf[33 * 16];  /* max T=33, nHidden=16, replace malloc */
    int16_t *y_rev = y_rev_buf;
    ...
    /* y_rev uses static buffer — no free needed */
}

/**
 * agc.c — 语音 AGC + 降噪联合模块 (v21 — linux_api17, 前置AGC)
 * ==================================================================
 * GTCRN linux_api19 方案: AGC先压动态 → 固定增益放大 → NR降噪
...
/* ── voice_NoiseReductionAndAGC: 前置AGC (GTCRN方案) ── */
    ...
    long long goal, short flag, int n_samples)
{
    ...
    case STATE_FLAG_AGC: {
#ifdef __cplusplus
        short local[512]; /* C++: VLA not supported, 512 is safe upper bound */
#else
        short local[n_samples];
#endif
        memcpy(local, in, n_samples * sizeof(short));
    ...
    case STATE_FLAG_BOTH: {
        /* ── 前置AGC: LMS压动态 → 固定增益放大 → NR降噪 ── */
#ifdef __cplusplus
        short agc_out[512]; /* C++: VLA not supported, 512 safe upper bound */
#else
        short agc_out[n_samples];
#endif
        memcpy(agc_out, in, n_samples * sizeof(short));
        int vf = g_v_inited ? 1 : 0;
        voice_agc_core(agc_out, agc_out, n_samples, (unsigned int)vf, goal);

        if (!g_first_frame) {
            int gn = (goal == AGC_GOAL_HIGH) ? AGC_GAIN_HIGH_NUM : AGC_GAIN_LOW_NUM;
            for (int i = 0; i < n_samples; i++) {
                int32_t v = (int32_t)agc_out[i] * gn / AGC_GAIN_DEN;
                agc_out[i] = s16_clamp(v);
            }
        }
        noise_reduction(agc_out, out, n_samples);
        break;
    }
